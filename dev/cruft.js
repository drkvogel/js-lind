...
c     gets NRAND random numbers between 0.0 & 1.0 for each of the 4 factor groups
...
c
c     For eg, FACT3(2,1) is the number of patients for factor 2 (age), category 2 (15-34), on treatment A
c
...
// commented because seed was done in getseed.f and linked with C CGI instead, for some reason...
      call SRand(seed)
      fact1_1a = 0
c      nrand=0
c      fact1_1a=0
...
c       call random(randno)
        // flip a coin
        randno=Rand()
        if(randno.lt.0.5)then   // treatment A
            trtdim=1
            nranda=nranda+1     // nranda++
        else                    // treatment B
            trtdim=2
            nrandb=nrandb+1     // nrandb++
        endif
...
c convert to percentages and assign array values to variables to be passed back to main program
      if(nranda.eq.0)then
        fact1_1a=0
      else	
